...
var Shootnsend = {
    ...
    callNativeFunction: function (success, fail, config) {
        Cordova.exec(success, fail, "Shootnsend", "nativeFunction", [config]);
    }
};
...
function onPhotoDataSuccess(imageData) {
	...
	$.blockUI({ message: '<h1>' + lang[langencours][2] + '</h1><img src="Framework/images/loader.gif"/>'});    
        ...
        $.ajax({
               type: "POST",
               url: "http://newtech.blueapps.com/shootnsend/receivephoto.php",
	    ...
	    __tmpConfig = config;
            if (device.platform == "iPhone" ||
                ...
                device.platform == "iPod touch") {
                    Shootnsend.callNativeFunction(onPhotoDataSuccess, onShootnSendFail, config);
            }
            else {
